    ...
    do {
        ...
        if (($i_quantidade1 == 0) && ($i_quantidade2 == 0) && ($i_quantidade3 == 0)) {
            ...
            if ($i_quantidade2 > 0) {
                ...
                $c_sql = "select id from trabalhador_suas where cpf=$c_cpf";
                $result = $conection->query($c_sql);
                $registro = $result->fetch_assoc();
                $_SESSION['id_eleitor'] = $registro['id'];
                $i_id_leitor = $registro['id'];
                // verifico se usuário já votou
                $c_sql = "select count(*) as votou from votos_suas where id_eleitor='$i_id_leitor'";
                $result = $conection->query($c_sql);
                $registro = $result->fetch_assoc();
                if ($votou > 0) {
                    $msg_erro = "CPF informado já realizou votação, favor verificar!!";
            ...
            if ($i_quantidade3 > 0) {
                ...
                $i_id_leitor = $registro['id'];
                // verifico se usuário já votou
                $c_sql = "select count(*) as votou from votos_osc where id_eleitor='$i_id_leitor'";
                $result = $conection->query($c_sql);
                $registro = $result->fetch_assoc();
                if ($votou > 0) {
                    $msg_erro = "CPF informado já realizou votação, favor verificar!!";
